    ...
    <Dialog open={isOpen} onOpenChange={() => onClose()}>
      <DialogContent className="sm:max-w-md bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 border-0 backdrop-blur-xl text-white shadow-2xl outline-none ring-0">
        ...
        <div className="flex gap-2 w-full">
          <Button 
            variant="outline" 
            className="flex-1 bg-[#1877F2]/20 border border-[#1877F2]/50 text-white hover:bg-[#1877F2]/30 flex items-center justify-center gap-2 py-2"
            onClick={async () => {
              setLoading(true);
              try {
                const { error } = await supabase.auth.signInWithOAuth({
          ...
          <Button 
            variant="outline" 
            className="flex-1 bg-[#0A66C2]/20 border border-[#0A66C2]/50 text-white hover:bg-[#0A66C2]/30 flex items-center justify-center gap-2 py-2"
            onClick={async () => {
              setLoading(true);
              try {
                const { error } = await supabase.auth.signInWithOAuth({
