...
import { Mail, Play, UserPlus, User } from 'lucide-react';
...
const VisitorAuth: React.FC<VisitorAuthProps> = ({ 
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      if (onGuestContinue) {
        onGuestContinue(validatedData.name, validatedData.email);
      }
      ...
      toast({
        title: "Welcome!",
        description: `Welcome, ${validatedData.name}! Sign up to unlock all features.`,
      });
      ...
      <DialogContent className="sm:max-w-md bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 border border-blue-500/30 backdrop-blur-xl text-white">
        <DialogHeader>
          ...
          <p className="text-center text-blue-200 text-sm">
            Continue as guest or sign in for full access
          </p>
        </DialogHeader>
