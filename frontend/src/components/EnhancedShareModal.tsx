...
const EnhancedShareModal: React.FC<EnhancedShareModalProps> = ({ 
  ...
  // Extract name from title for viral text
  const extractedName = title?.replace(/^Check out /i, '').replace(/'s profile$/i, '') || 'someone amazing';
  const viralShareText = `🚀 Just discovered ${extractedName}'s AI-powered profile on AvatarTalk.Co — this is the future of personal branding! Check it out ✨🔥`;

  // Draw branded logo on QR (matching dashboard ShareModal)
  ...
  useEffect(() => {
    ...
    qrBuildingRef.current = false;
    const id = requestAnimationFrame(() => generateBrandedQR());
    return () => cancelAnimationFrame(id);
  }, [isOpen, generateBrandedQR]);

  useEffect(() => {
    if (view !== 'qr' || qrReadyRef.current || qrPngUrl) return;
    generateBrandedQR();
  ...
  const downloadQRCode = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      link.download = `avatartalk-${type}-qr.png`;
      link.href = url;
      link.click();
  ...
  const shareToSocial = (platform: string) => {
    ...
    const fullText = encodeURIComponent(`${viralShareText}\n${shareUrl}`);
    ...
    const urls: Record<string, string> = {
      ...
      gmail: `mailto:?subject=${encodeURIComponent(`${extractedName}'s profile on AvatarTalk.Co`)}&body=${encodeURIComponent(`${viralShareText}\n\n${shareUrl}`)}`,
        ...
        <AnimatePresence mode="wait">
          {view === 'share' ? (
            ...
            >
              ...
              {/* Viral share text preview */}
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-100 rounded-lg p-3">
                <p className="text-xs text-gray-600 font-medium mb-1">Share text:</p>
                <p className="text-sm text-gray-800 leading-relaxed">{viralShareText}</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Input
                    value={shareUrl}
                    readOnly
                    className="bg-gray-50 border-gray-200 text-gray-800 flex-1 text-sm"
                  />
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={copyToClipboard}
                    className={`border-gray-200 ${copied ? 'bg-green-50 text-green-700 border-green-300' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                ...
                <div className="grid grid-cols-4 gap-2 max-h-[280px] overflow-y-auto pr-1">
                  {socialPlatforms.map((platform) => (
                    <motion.button
                      ...
                      className={`flex flex-col items-center gap-1.5 p-2.5 rounded-xl bg-gradient-to-br ${platform.color} text-white hover:shadow-lg transition-all duration-200`}
                    >
                      {platform.icon}
                      <span className="text-[10px] font-medium leading-tight">{platform.label}</span>
                    </motion.button>
                  ))}
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <Button variant="ghost" size="sm" onClick={() => setView('share')} className="text-gray-600 hover:bg-gray-100">
                  <ArrowLeft className="w-4 h-4 mr-1" /> Back
                </Button>
                <h3 className="text-lg font-bold text-gray-900">QR Code</h3>
              </div>

              <div className="bg-white border border-gray-100 shadow-md p-4 rounded-xl inline-block">
                {qrReady && qrPngUrl ? (
                  <img src={qrPngUrl} alt="Branded QR Code" className="w-60 h-60 mx-auto" />
                ) : (
              ...
              <p className="text-sm text-gray-500">
                Scan to visit the {type} • Powered by AvatarTalk.Co
              </p>
