...
const SocialLinksManager: React.FC = () => {
  ...
  const saveSocialLinks = async () => {
    ...
    try {
      ...
      // If RLS blocked the upsert, fall back to explicit insert/update
      ...
      if (/row-level security|permission denied|policy/i.test(raw)) {
        description = 'Permission denied. Apply migration `20260301000000_avatartalk_fixes.sql` in Supabase SQL editor.';
