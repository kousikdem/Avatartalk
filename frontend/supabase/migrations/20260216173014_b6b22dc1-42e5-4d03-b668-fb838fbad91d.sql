...
-- FIX: Add authorization check to transfer_tokens to prevent token theft
...
BEGIN
  -- CRITICAL: Verify the caller IS the sender (prevent token theft)
  IF auth.uid() IS NULL OR auth.uid() != p_sender_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Unauthorized: You can only transfer from your own account');
  END IF;

  -- Prevent self-transfer
  IF p_sender_id = p_receiver_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Cannot transfer tokens to yourself');
  END IF;
